...
const SAMPLE_REPLIES: Reply[] = [
  {
    id: 'r1',
    ventId: 'v1',
    content: "spotify unfollow is personal yrr 💀 he is definitely hurt. change your playlist name to something pettier to assert dominance 💅🏻",
    createdAt: new Date(Date.now() - 1000 * 60 * 10).toISOString(),
  },
  {
    id: 'r2',
    ventId: 'v2',
    content: "bhai your friend is playing with fire, but also why are we like this 😭 grab some tea, let her overthink, you please sleep yrr",
    createdAt: new Date(Date.now() - 1000 * 60 * 30).toISOString(),
  },
  {
    id: 'r3',
    ventId: 'v3',
    content: "not you planning the wedding over a water bottle 💀 bestie please seek help (but lowkey what pattern is the registry? list me down)",
    createdAt: new Date(Date.now() - 1000 * 60 * 100).toISOString(),
  },
  {
    id: 'r4',
    ventId: 'v4',
    content: "roommate milk switch is pure evil yrr 😭 get a lock for your carton or drink oat milk straight from the cow next time",
    createdAt: new Date(Date.now() - 1000 * 60 * 150).toISOString(),
  },
  {
    id: 'r5',
    ventId: 'v5',
    content: "a script writer is crazy 😭 he has a template for relationships. you should comment 'recycle emoji' on it yrr",
    createdAt: new Date(Date.now() - 1000 * 60 * 250).toISOString(),
  },
  {
    id: 'r6',
    ventId: 'v6',
    content: "real talk, parents just need some updates to feel in control. take a breath bro, we are all figuring it out one day at a time 🤍",
...
export const mockDb = {
  ...
  async getVents(options?: { mood?: string; page?: number; limit?: number; sort?: string; replyStyle?: string }): Promise<Vent[]> {
    ...
    filtered = filtered.filter(v => {
      if (['v1', 'v2', 'v3', 'v4', 'v5', 'v6', 'v7', 'v8', 'v9', 'v10'].includes(v.id)) return true;
  ...
  async updateReplyContent(replyId: string, content: string): Promise<Reply | undefined> {
